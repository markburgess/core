...
Body *IsBody(Body *list, const char *namespace, const char *key)
{
    ...
    // bp->namespace is where the body belongs, namespace is where we are now
        ...
        if (strcmp(namespace,bp->namespace) != 0)
        {
            continue;
        }
        ...
        else if (strncmp(namespace,key,strlen(namespace)) == 0)
        {
            strcpy(fqname,key);
        }
        else
        {
            snprintf(fqname,CF_BUFSIZE-1, "%s_%s",namespace,key);
        }
...
Bundle *IsBundle(Bundle *list, const char *key)
{
    ...
    for (bp = list; bp != NULL; bp = bp->next)
    {
        ...
        {
            snprintf(fqname,CF_BUFSIZE-1, "%s_%s",bp->namespace,key);
        }
